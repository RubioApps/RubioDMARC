...
if ($folder = opendir(OD_INCLUDES)) {
    while (false !== ($file = readdir($folder))) {
        if ($file != "." && $file != ".." && $file != 'factory.php') {
            require_once OD_INCLUDES . DIRECTORY_SEPARATOR . $file;
        }
    }
    closedir($folder);
}
...
class Factory
{
    ...
    public static function getDatabase()
    {
        ...
        if (!static::$database) {
            static::$database = new Database([
                'host' => static::$config->database['host'],
                'port' => static::$config->database['port'],
                'user' => static::$config->database['user'],
                'password' => static::$config->database['password'],
                'database' => static::$config->database['database'],
            ]);
        }
    ...
    protected static function _restorePrefs()
    {
        ...
        if (!count(static::$prefs)) {
            ...
            if (empty(static::$prefs['mode']))
                self::_defPreference('mode', 'dark', 'string');

            // Basic pref language
            if (empty(static::$prefs['hl']))
                $language = new Language();
            self::_defPreference('hl', $language->detectLanguage(), 'string');
    ...
    public static function getParams()
    {
        ...
        static::$params['hl'] = 'en-GB';
        static::$params['mode'] = 'dark';
    ...
    {
        if (file_exists(OD_MODELS . DIRECTORY_SEPARATOR . strtolower($name) . '.php')) {
            require_once(OD_MODELS . DIRECTORY_SEPARATOR . strtolower($name) . '.php');

            $classname  = '\RubioVPN\Framework\model' . ucfirst($name);
    ...
    public static function jsBridge()
    {
        ...
        switch ($task) {
            ...
            case 'language':
                $language = new Language();
                $hl = Request::getVar('hl', $language->detectLanguage(), 'GET');
                self::setLanguage($hl);
                $source = ['success' => true, 'message' => $hl];
                break;
            case 'token':
