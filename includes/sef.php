...
class SEF
{
    protected static $task = 'home';
    protected static $hl   = 'en-GB';
    ...
    public static function _($uri)
    {
        ...
        foreach ($array as $key => $value) {
            ...
            //Set the pathway for a given task if there is another parameter
            if (count($array) && $key == 'task' && empty($pathway)) 
                array_push($pathway,$value);
        ...
        $ret  = $config->live_site . '/' . Request::getVar('hl', Factory::getLangTag(), 'GET');
        if (count($trail))
            $ret .= '/' . join('/', $trail);
    ...
    public static function parseURI()
    {
        ...
        // At root level without a language trail
        if (!count($trail)) {
            if (!strlen($_SERVER['QUERY_STRING'])) {
                header('Location: ' . $config->live_site . '/' . $params['hl'] . '/');
                exit(0);
            }
            return;
        } else {
            $hl = array_shift($trail);
            if ($hl !== $params['hl']) {
                array_unshift($trail, $params['hl']);
                Factory::setLanguage($hl);
                header('Location: ' . $config->live_site . '/' . join('/', $trail) . '/');
                exit(0);
            }
        }

        // Get task
        $task = count($trail) ? array_shift($trail) :  'home';

        // Default trail pattern
        $props = ['id'];
    ...
    public static function save($pathway, $id, $alias)
    {
        ...
        $filepath   = OD_SEF . DIRECTORY_SEPARATOR . $pathway . '.xml';
